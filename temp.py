...
def build_config():
    ...
    parser.add_argument("--n_epochs", dest="n_epochs", default=20)
    parser.add_argument("--early_stop", dest="early_stop", default=2)
    parser.add_argument("--target", dest="target", default="model")
...
################################################################################################
    ...
    from model import SeqModel3
    from step_trainer import step_Trainer
    ########## STEMP2
    if not logging.getLogger() == None:
        for handler in logging.getLogger().handlers[:]:  # make a copy of the list
            logging.getLogger().removeHandler(handler)

    logging.basicConfig(filename='train2_log', level=LOGFILE_LEVEL)  # logging의 config 변경
    console = logging.StreamHandler()  # logging을 콘솔화면에 출력
    console.setLevel(CONSOLE_LEVEL)  # log level 설정
    logging.getLogger().addHandler(console)  # logger 인스턴스에 콘솔창의 결과를 핸들러에 추가한다.
    ...
    config.target = 'model2'
    _print_config(config)

    crits = nn.NLLLoss()
    trainer = step_Trainer(model, crits, config, options)
    ...
    config.target = 'model3'
    config.hidden_size = 512
    config.word_vec_dim = 512
    _print_config(config)
